...
def generate_parameter_combinations(parameters, sample_num, ele_around_circum):
            ...
            for param, values in defect[1].items():
                if f'{defect_type}_{param}' not in parameters:
                    if 'phi' in param:
                        ...
                        parameters[f'{defect_type}_{param}'] = list(map(float, values))
                else:
                    added = False
                    i = 1
                    while added == False:
                        if f'{defect_type}_{param}_{i}' not in parameters:
                            if 'phi' in param:
                                ...
                                parameters[f'{defect_type}_{param}_{i}'] = list(map(float, values))
                            added = True
                        else:
                            continue
        ...
        if len(parameters) == 0 and len(constant_params) != 0:
            combinations_list = []
            for i in range(sample_num):
                combinations_list.append(list(zip(constant_params.keys(), constant_params.values())))
